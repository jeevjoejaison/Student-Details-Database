...
import { CulturalEventForm } from "@/components/forms/CulturalEventForm";
...
const CulturalEventPage = () => {
  ...
  useEffect(() => {
    if (activeTab === "view" && user?.userId) {
      fetchFormData("cultural-events", user.userId).then(setEvents);
    }
    console.log(events)
  ...
  return (
    <MainLayout title="Cultural Events" showBackButton>
      <div className="space-y-6 bg-gradient-to-r from-purple-50 to-blue-50 p-6 rounded-lg shadow-lg">
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-3xl font-bold text-purple-900">Cultural Events</h2>
            <p className="text-purple-600">Manage your cultural event participation and achievements</p>
          </div>
        </div>
        ...
        <Tabs defaultValue="add" value={activeTab} onValueChange={setActiveTab} className="w-full">
          ...
          <TabsContent value="add" className="pt-4">
            <CustomCard className="bg-white border border-purple-200 p-6 shadow-lg rounded-lg">
              <h3 className="text-lg font-medium text-purple-900 mb-4">Add Cultural Event</h3>
              <CulturalEventForm />
            </CustomCard>
          </TabsContent>
            ...
            {events.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {events.map(event => (
                  <CustomCard key={event.activityId} className="p-6 bg-white border border-purple-200 shadow-lg rounded-lg hover:shadow-xl transition-shadow">
                    ...
                    <p className="text-sm text-purple-600">Awards: {event.awards}</p>
                    ...
                    <div className="flex gap-2 mt-4">
                      ...
                      <Button 
                        onClick={() => handleDelete("cultural-events",event.activityId)} 
              ...
              <CustomCard className="flex flex-col items-center justify-center py-10 bg-white border border-purple-200 shadow-lg rounded-lg">
                <div className="text-center space-y-3">
                  ...
                  <h3 className="text-lg font-medium text-purple-900">No cultural events added yet</h3>
                  <p className="text-sm text-purple-700 max-w-md mx-auto">
                    When you add cultural events, they will appear here. Add your first cultural event to get started.
                  </p>
                  <Button 
